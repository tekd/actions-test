...
const d3Chart = {
  ...
  createBarChart(chart) {
    ...
    console.log(subgroups, 'sg');
    let headers = data.columns;

    // List of groups = species here = value of the first column called group -> I show them on the X axis
    let groups = d3
      .map(data, function(d) {
        return d[0]; //requires the x axis to be the first column
      })
      .keys();
  ...
  svg.append("g")
    ...
    .data(stackedData)
    .enter().append("g")
      .attr("fill", function(d) { return chartAttrs.color(d.key); })
      .selectAll("rect")
      // enter a second time = loop subgroup per subgroup to add all rectangles
      .data(function(d) { return d; })
      .enter().append("rect")
        .attr("x", function(d) { return x(d.data[headers[0]]); })
